...
const agentIdOption: OptionContract = { flags: "--agent-id <id>", description: "Agent ID" };

export function registerAgentCommands(parent: Command, registry: CommandRegistry, app: AppContext): void {
  const agents = registry.group(parent, "agents", "Create and manage company Agents");

  registry.leaf(agents, {
    path: "agents create",
    description: "Create a company Agent and optionally its Capability draft",
    options: [
      { flags: "--identifier <identifier>", description: "stable Agent identifier" },
  ...
  registry.leaf(agents, {
    ...
    await app.simple(global, {
      ...
      query: compactQuery({ status: text(options.status), search: text(options.search) })
    }, "AgentList");
  });
  ...
  registry.leaf(agents, {
    ...
    await app.simple(global, { method: "GET", path: `${app.managementPrefix(current.profile)}/agents/${escape(agentId)}` }, "Agent");
  });
  ...
  registry.leaf(agents, {
    ...
    options: [
      ...
      { flags: "--description <description>", description: "Agent description" },
      { flags: "--owner-user-id <id>", description: "Agent owner user ID" },
      { flags: "--version <number>", description: "Agent record version", defaultValue: "1" }
    ]
  }, async (options, command) => {
    const global = app.global(command);
    const current = await app.currentProfile(global);
  ...
  const capability = registry.group(agents, "capability", "Manage Agent Capability drafts");
  registry.leaf(capability, {
  ...
  for (const action of ["submit", "withdraw"] as const) {
    registry.leaf(capability, {
      ...
      options: [
        agentIdOption,
        { flags: "--version <number>", description: "Capability draft version", defaultValue: "1" },
        ...(action === "withdraw" ? [
          { flags: "--reason <reason>", description: "withdrawal reason" },
      ...
      const global = app.global(command);
      const current = await app.currentProfile(global);
      if (action === "withdraw") confirmation(options, "withdraw Agent approval request");
      const body: Record<string, unknown> = { version: integerOption(options.version, "version", 1) };
      if (action === "withdraw") body.reason = requiredText(options.reason, "reason");
      await app.simple(global, {
        method: "POST",
  ...
  const lifecycle = registry.group(agents, "lifecycle", "Pause, resume, or archive an Agent");
  for (const action of ["pause", "resume", "archive"] as const) {
    registry.leaf(lifecycle, {
      ...
      if (current.profile.login_type === "user" && action !== "archive") {
        throw new CliError({ code: "ADMIN_LOGIN_REQUIRED", message: "only a tenant administrator can pause or resume an Agent", exitCode: 2 });
      }
...
async function createAgent(app: AppContext, options: OptionValues, command: Command): Promise<void> {
  const global = app.global(command);
  const current = await app.currentProfile(global);
  ...
  if (file !== "") {
    ...
    if (first) {
      ...
      if (filePolicies.length === 0) filePolicies = stringList(first.permission_ids);
      if (filePolicies.length === 0) filePolicies = permissionIds(first.permissions);
    }
  }
  if (name !== "") {
    identifier = name;
  ...
  if (filePolicies.length > 0 && cliPolicies.length > 0) {
    if (Boolean(options.replacePermissions) === Boolean(options.appendPermission)) {
      throw new CliError({ code: "AMBIGUOUS_PERMISSION_MERGE", message: "file and command-line permissions require exactly one merge option", exitCode: 2 });
    }
    policies = options.replacePermissions ? cliPolicies : uniqueStrings([...filePolicies, ...cliPolicies]);
  } else if (filePolicies.length > 0) {
    policies = filePolicies;
  }
  if (!global.nonInteractive && file === "") {
    identifier = await promptIfEmpty(app.io.input, app.io.error, "Agent identifier", identifier);
    displayName = await promptIfEmpty(app.io.input, app.io.error, "Agent display name", displayName);
    applicationId = await promptIfEmpty(app.io.input, app.io.error, "GenAuth application ID", applicationId);
  }
  ...
  const agent = decodeResponseData<{ id?: string; version?: number }>(created.data);
  if (!agent.id) {
  ...
  if (audience !== "" || policies.length > 0) {
    ...
    try {
      const grant = await app.call(global, {
        ...
        body: { audience, data_policy_ids: policies, permission_snapshot: {}, version: 0 },
        headers: idempotencyHeaders()
      });
      app.success(global, "AgentWithCapabilityDraft", { agent: created.data, capability_grant: grant.data }, grant.requestId);
      return;
    } catch (error) {
      const cause = error instanceof CliError ? error : invalid("Capability draft failed");
      throw new CliError({
        code: "PARTIAL_AGENT_CREATE",
        message: "Agent was created, but its Capability draft could not be saved",
...
function registerSettings(agents: Command, registry: CommandRegistry, app: AppContext): void {
  const settings = registry.group(agents, "settings", "Manage Agent-level authorization and Token settings");
  registry.leaf(settings, {
    ...
    const global = app.global(command);
    const current = await app.currentProfile(global);
    await app.simple(global, { method: "GET", path: `${app.managementPrefix(current.profile)}/agents/${escape(requiredText(options.agentId, "agent-id"))}/settings` }, "AgentSettings");
  });
  registry.leaf(settings, {
    ...
    options: [
      agentIdOption,
      { flags: "--file <path>", description: "complete settings YAML or JSON file" },
      { flags: "--authorization-mode <mode>", description: "explicit-only or silent-if-allowed" },
      { flags: "--token-ttl <duration>", description: "Agent access Token TTL" },
      { flags: "--max-user-grant-ttl <duration>", description: "maximum UserGrant TTL" },
      { flags: "--redirect-uri <uri>", description: "allowed authorization redirect URI", collect: true },
      { flags: "--require-realtime-decision", description: "require a current GenAuth decision", defaultValue: true },
      { flags: "--credential-ttl <duration>", description: "Agent Credential TTL" },
      { flags: "--rotation-overlap <duration>", description: "Credential rotation overlap" },
      { flags: "--version <number>", description: "expected existing draft record version", defaultValue: "0" }
    ]
  }, async (options, command) => {
    const global = app.global(command);
    const current = await app.currentProfile(global);
    ...
    if (file !== "") {
      const changed = ["authorizationMode", "tokenTtl", "maxUserGrantTtl", "redirectUri", "credentialTtl", "rotationOverlap"]
        .some(name => command.getOptionValueSource(name) === "cli");
      if (changed) throw new CliError({ code: "AMBIGUOUS_SETTINGS_INPUT", message: "--file is exclusive with Agent settings value flags", exitCode: 2 });
      body = await readObjectFile(file);
    } else {
      const mode = text(options.authorizationMode).replaceAll("-", "_").toUpperCase();
      if (mode !== "EXPLICIT_ONLY" && mode !== "SILENT_IF_ALLOWED") throw invalid("authorization-mode must be explicit-only or silent-if-allowed");
      const tokenTtl = positiveDuration(options.tokenTtl, "token-ttl");
      const maxGrantTtl = positiveDuration(options.maxUserGrantTtl, "max-user-grant-ttl");
